...
let cities = {
  parisCity: {
    ...
    cityDescription:
      "This is the fantastic description about the city of paris",
    cityPlacesToVisit: [
      {
        place: "Eiffel tower",
        image: "./assets/images/paris_eiffel_tower.webp",
        description:
          "The iconic symbol of Paris, the Eiffel Tower offers breathtaking \
        	views of the city from its observation decks. Completed in 1889, \
			it stands as a testament to architectural innovation and attracts \
      ...
      },
      {
        ...
        description:
          "Home to thousands of masterpieces, including the Mona Lisa and the\
			Venus de Milo, the Louvre is the world’s largest art museum.\
			Its stunning glass pyramid entrance is a modern architectural\
			landmark in the heart of Paris.",
        findOutMore: "https://en.wikipedia.org/wiki/Louvre",
        alt: "The louvre located in paris, houses in the background and the\
			beautiful blue sky",
      },
      {
        ...
        description:
          "This Gothic masterpiece, located on the Île de la Cité, is renowned\
			for its stunning architecture, rose windows, and historic \
			significance. Though under restoration after the 2019 fire, it \
			remains a symbol of Parisian heritage.",
        findOutMore: "https://en.wikipedia.org/wiki/Notre-Dame_de_Paris",
        alt: "The notre dame cathedral with trees on the right and \
			the beautiful sky",
      },
      {
        ...
        description:
          "is a Roman Catholic church on Montmartre hill, known for its striking\
			white domes. Completed in 1914, it features a large mosaic inside \
			and offers panoramic views of Paris from its terrace. It’s a symbol\
			 of hope and a popular site for both worship and tourism.",
        findOutMore:
          "https://en.wikipedia.org/wiki/Sacr%C3%A9-C%C5%93ur,_Paris",
        alt: "The Sacré-Cœur Basilica with refreshingly green grass out front \
  ...
  londonCity: {
    ...
    cityDescription:
      "This is the fantastic description about the city of london",
    cityPlacesToVisit: [
      {
        place: "Big Ben",
        image: "./assets/images/london_big_ben.jpg",
        description:
          "Big Ben, officially the Elizabeth Tower, is an iconic symbol of\
			London at the Palace of Westminster. Built in 1859, its towering\
			Gothic design and clock face are globally recognized, offering a\
			classic view from Westminster Bridge.",
      ...
      },
      {
        ...
        description:
          "The British Museum, located in Bloomsbury, is one of the world’s\
			largest museums, showcasing human history, art, and culture. Opened\
			in 1753, it houses iconic artifacts like the Rosetta Stone and the\
			Elgin Marbles. With free entry, it offers a fascinating journey \
			through over two million years of history.",
        findOutMore: "https://en.wikipedia.org/wiki/British_Museum",
        alt: "The british museum and the dreadful british weather on display \
		for all to see ",
      },
      {
        ...
        description:
          "The Tower of London, a historic fortress on the Thames, has been a\
			royal palace, prison, and treasury since 1066. It’s home to the\
			Crown Jewels and guarded by the Yeoman Warders. Visitors can explore\
			its medieval structures and learn about its rich and sometimes eerie\
			past.",
        findOutMore: "https://en.wikipedia.org/wiki/Tower_of_London",
        alt: "The tower of london, the thames river out front and surrounded by\
		trees",
      },
      {
        place: "Covent Garden",
        ...
        description:
          "Covent Garden, in London’s West End, is a lively district famous for\
			its historic market, boutique shops, and vibrant street\
			performances. It’s a hub for entertainment, featuring the Royal\
			Opera House and an array of restaurants and theaters.",
        findOutMore: "https://en.wikipedia.org/wiki/Covent_Garden",
        alt: "A picture inside covent garden displaying the beauty of the\
...
let userDetails = {
  ...
  /* If a user approves location permissions successCallBack will be invoked,
  this will update and centre the map position to the users location */
  ...
  errorCallBack: () => {
    ...
    if (isHomePage) {
      Swal.fire({
        icon: "error",
        title: "Oops...",
        text: "If user location permissions are denied you will not be able to\
			see your current location marker on the map.",
      });
    }
    initMap();
  },
};
/* On click of a city or on approval of user location permissions this will
update the chosen cities position attributes */
  ...
  if (mapIsPresent !== null) {
    ...
    if (locationAllowed) {
      const marker2 = new AdvancedMarkerElement({
        map: map,
        position: {
          lat: userDetails.currentUserLatitude,
...
async function handleCityClick(e) {
  ...
  notation on its own was not able to make the code work through various
   ...
   but could not find any other way of making this work */
  chosenCityDetails.chosenCityLatitude =
    cities[e.target.closest(".cityCard").id].cityLatitude;
  chosenCityDetails.chosenCityLongitude =
    cities[e.target.closest(".cityCard").id].cityLongitude;
  ...
  for (place of cities[cityId].cityPlacesToVisit) {
    cityInformationContainer.innerHTML += `
      <div class="card mb-3" style="max-width: 740px;">
        <div class="row g-0">
          <div class="col-md-4">
            <img src="${place.image}" class="img-fluid rounded-start 
            placesToVisitCardImg h-100" alt="${place.alt}">
          </div>
          <div class="col-md-8">
            <div class="card-body">
...
/** Adds the cityActive class if it is not present, removes it if it is present
and removes the class from elements other than the selected one, if it is found.
 *
 ...
 *below each
 *
 * @param {Submit} e - This is information of the event that triggers the 
function
 */
...
/** Places an outline on the selected city card and removes previously selected
city card outlines.
 * @param {ClickEvent} e - This is information of the event that triggers the
 function
 */
...
/* NAKED CODE, no specific organisation for this code and does not fit into 
other categories */
